...
def main(cfg: DictConfig) -> None:
    ...
    model = DiT(
        ...
        in_channels=4,
        hidden_size=cfg.model.hidden_size,  # 192,#1152,
        depth=cfg.model.depth,  # 4
        num_heads=cfg.model.num_heads,  # 6
    ...
    for step in range(epochs):
        ...
        for substep, batch in enumerate(train_loader):
            ...
            for t in range(start, t_dim - 1):
                ...
                # try to condition with a noisy version
                ...
                noise_factor = scheduler.alphas_cumprod.to(x.device)[k]
                ...
                y_noised = scheduler.add_noise(y, noise, k)
                # x_in = torch.cat([images[..., t], y_noised], axis=-1)
                # with torch.autocast(device_type='cuda', dtype=torch.float16):
                pred = model(torch.cat([x, y_noised], axis=1), k * time_multiplier)
            ...
            for substep, batch in enumerate(test_loader):
                ...
                with torch.no_grad():
                    ...
                    for t in range(t_dim - 1 - start):
                        y_noised = torch.randn_like(x).to(
                            x
                        )  # , dtype=x.dtype, device=x.device
                        for k in scheduler.timesteps:
                            time = (
                                torch.zeros(
                                    size=(x.shape[0],), dtype=x.dtype, device=x.device
                                )
                                + k
                            )
                            pred = model(
                                torch.cat([x, y_noised], axis=1), time * time_multiplier
                            )
