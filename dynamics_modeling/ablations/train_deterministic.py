...
def main(cfg: DictConfig) -> None:
    ...
    run.tags = ("deterministic",) + (dataset_name,) + (f"sub={sub_tr}",)

    trainset = TemporalDataset(
        u_train, grid_tr, latent_dim, dataset_name
    )
    trainset_extra = TemporalDataset(
        u_train_eval, grid_tr_extra, latent_dim, dataset_name
    )
    testset = TemporalDataset(
        u_test, grid_te, latent_dim, dataset_name
    )
